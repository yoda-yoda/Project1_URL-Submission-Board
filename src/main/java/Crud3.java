...
public class Crud3 {
    ...
    public static void main(String[] args) {
        ...
        userSignUpDay.add(LocalDate.now()); // 가입일 저장소에 가입일 저장.
        userEditDay.add(null); // 계정 수정일 저장소에 인덱스를 맞추기위해서 null로 저장.
        // --- 관리자 계정 초기에 추가 ---

        Session<String> session = new Session<>(null); // 유저 로그인 여부와 정보를 담는 session 객체를 최상위에서 생성
        ...
        do {
            ...
            temporaryParameterSplit = null;  // 기능이 한번끝나서 다시 다음에 명령어에 재사용하기위해 null로 안의 값들을 삭제.
            ...
            if (!userInput.equals("종료")) {
                ...
                userInputPath = userInput.split("/",3); // 유저의 입력중에 "/"를 기준으로 3덩이 까지만 나눠서 저장함.
                // 테스트결과 이 변수에 담는것까진 "/"든, 공백이든 뭐라고 입력해도 오류가안남.
                ...
                try {
                    ...
                    firstCheck = true;
                    // 여기서 오류가 안나야지 밑 if조건이 충족된다. try안의 오류코드다음은 아예실행 안되는것을 활용해서 boolean 과 같이 사용했다.
                ...
                if (firstCheck && userInputPath.length == 3 && userInputPath[0].equals("") &&
                        !userInputPath[1].equals("") && !userInputPath[2].equals("") ){ // 여기 진입하는 입력은 최소한 [/abc/abc]
                    ...
                    if (userInputPath[1].equals("boards") && userInputPath[2].equals("add") ) {
                        //  게시판 작성. 예를들어 입력이 /boards/add   일경우에만 진입할것이다.
                        // 맨 윗줄에서 userInputPath = userInput.split("/",3); 처럼  limit을 3 설정하면서 뒤의 덩어리자체를 저장시켰기에 필터링이 가능해졌다. /를 더 추가해도 필터링된다.

                        if(session.masterLogin){ //관리자만 진입가능.
                            ...
                            for(int i=0; i < boardKeyStorage.size(); i++){ // 이름이 기존게시판과 같은지 체크하기위한 for문.
                                ...
                                if( userInput.equals(boardKeyStorage.get(i)) ) {   // 사이즈0이면 for자체를실행안하니까 안에서 예외처리는 안해도된다.
                                    existingName = true; // 기존 게시판 이름이 있으면 생성에 진입못하게만듬.

                                    System.out.println();
                                    System.out.println("이미 해당 게시판이 존재합니다.");

                                    elseCheck1 = true; // 맨밑에서 유효하지않은 URL 이라는 출력을 막기위함.
                                    break;
                                }
                            ...
                            if(!existingName){
                                ...
                                boardWriter.add(session.loginingAccount);
                        ...
                        } else { //관리자 로그인중이 아니라면진입.
                            System.out.println("게시판 작성 권한이 없습니다.");
                            elseCheck1 = true;
                        }
                            ...
                            if (okCheck1) { // 유저URL 입력이 =>   /boards/view?boardName=aaa&boardName=bbb ... 와 같은 입력만 여기에 진입함.
                                ...
                                if (okCheck2) { // 드디어 진입. /boards/view?boardName=...&boardName=자유게시판.. 처럼 입력했고, 입력한 그 마지막 게시판 키가 실존해야 진입가능.
                                    ...
                                    if (writeNumber != 0) {
                                        ...
                                        System.out.printf("[%s] 생성인 :%s",userBoardIdValueString, boardWriter.get(boardKeyIndex));
                                        System.out.println();
                        ...
                        if(session.masterLogin || session.userLogin){  // 유저이든 관리자든 로그인 된상태에서만 진입가능.
                                ...
                                if (okCheck1) { // 유저URL 입력이 =>   /posts/add?boardId=aaa&boardId=bbb ... 방식과 같은 입력만 여기에 진입함. &가 없어도됨.
                                    ...
                                    if (okCheck2) { //오류가 안나야 true. try블록안에서 오류코드 다음의 코드는 진행이 안되는것을 활용.
                                        ...
                                        if (userBoardIdValueInteger > 0 && userBoardIdValueInteger <= mapKeyStorage.size()) { // 드디어 진입. /posts/add?boardId=1 같은 형식과 같음.
                                            ...
                                            postWriter.get(userBoardIdValueInteger - 1).add(session.loginingAccount); // 로그인중인 계정명을 postWriter에 저장.
                            ...
                            userInputCrud[0].equals("remove") && userInputCrud.length == 2) { //   해당 게시판의 게시글삭제 진입시도. 로그인 안되어있으면 진입불가.
                                        ...
                                        if (okCheck1) { // 유저URL 입력이 =>   /posts/remove?postId=aaa&boardId=bbb ... 방식과 같은 입력만 여기에 진입함.
                                            ...
                                            if (okCheck2) { //오류가 안나야 true. try블록안에서 오류코드 다음의 코드는 진행이 안되는것을 활용.
                                                ...
                                                if (userBoardIdValueInteger > 0 && userPostIdValueInteger > 0 &&
                                                    ...
                                                    if( session.masterLogin ||
                                                            postWriter.get(userBoardIdValueInteger-1).get(userPostIdValueInteger-1).equals(session.loginingAccount) ){
                                                        // 첫째: 관리자이거나, 둘째: 해당 postWriter 가, 지금 로그인해둔 계정명이 같을때에만 진입가능.
                                                        // 그러면 이제 비로소 해당 게시판의 해당 게시글 삭제다.
                                                    ...
                                                    } else { // 관리자도 아니고,  게시글 작성자도 일치하지 않는다면 여기 진입.
                        ...
                        if(!session.userLogin && !session.masterLogin) { // 일반회원이든 관리자든 로그인 안된 상태에서만 진입.
                            ...
                            if(existingAccount){ // 입력 계정명이 실존하면 진입.
                                ...
                                if( inputPassword.equals(userPassword.get(userAccountIndex)) ){ // 해당 계정의 패스워드가 맞으면 진입. 계정보관소 인덱스와 패스워드 보관소의 인덱스가 같은걸 활용.
                                    ...
                                    if(!inputAccount.equals("관리자")){ // "관리자" 계정이 아닌 일반회원으로 로그인한 경우 진입.
                                        ...
                                        session.loginingAccount = inputAccount; // 로그인한 일반 계정명을 담아둬서 활용할수있도록함.
                                        elseCheck1 = true; // 유효하지앟은 URL 출력처리를 막기위함.
                                        ...
                                        session.loginingAccount = inputAccount; // 로그인한 관리자 계정명을 담아둬서 활용할수있도록함.
                                        elseCheck1 = true; // 유효하지앟은 URL 출력처리를 막기위함.
                        ...
                        if(session.userLogin || session.masterLogin ){ // 회원만 가능. 즉 이 시점에 계정 로그인이 되어있는 상태일때만 진입가능.

                            if(session.userLogin){ // 일반 유저로그인상태라면 즉 userLogin == true 상태라면 진입.

                                session.userLogin = false; // 로그아웃 상태로 바꾼다.
                                System.out.printf("[%s] 계정에서 로그아웃 되었습니다.", session.loginingAccount);
                                session.loginingAccount = null; // 로그아웃했으니 초기값 null로 재할당.
                                System.out.println();

                                elseCheck1 = true; // 유효하지앟은 URL 출력처리를 막기위함.

                            } else { // 관리자 로그인상태라면 즉 masterLogin == true 상태라면 진입.

                                session.masterLogin = false; //  로그아웃 상태로 바꾼다.
                                System.out.printf("[%s] 계정에서 로그아웃 되었습니다.", session.loginingAccount);
                                session.loginingAccount = null; // 로그아웃했으니 초기값 null로 재할당.
                                System.out.println();
                        ...
                        if( session.masterLogin || session.userLogin ) { // 관리자 또는 유저가 로그인 상태여야만 진입가능.
                                ...
                                if (okCheck1) { // 유저URL 입력이 =>   /posts/add?boardId=aaa&boardId=bbb ... 방식과 같은 입력만 여기에 진입함. &가 없어도됨.
                                    ...
                                    if (okCheck2) { //오류가 안나야 true. try블록안에서 오류코드 다음의 코드는 진행이 안되는것을 활용.
                                        ...
                                        if (accountIdValueInteger > 0 && accountIdValueInteger <= userAccount.size()) { // 드디어 진입. /accounts/edit?accountId=1 같은 형식과 같음.
                                            ...
                                            if( session.loginingAccount.equals(userAccount.get(accountIdValueInteger-1)) ) {
                                                // 수정하려는 계정명이랑 현재 로그인한 계정명이 일치해야만 진입.
                                                ...
                                                System.out.print("바꿀 이메일을 입력해주세요 :");
                                                ...
                                                System.out.println("계정 정보 수정 권한이 없습니다.");
                                                elseCheck1 = true;
                                            }
                        ...
                        if(session.masterLogin || session.userLogin ) { //로그인되어있어야만 진입가능.
                                ...
                                if (okCheck1) { // 유저URL 입력이 =>   /posts/add?boardId=aaa&boardId=bbb ... 방식과 같은 입력만 여기에 진입함. &가 없어도됨.
                                    ...
                                    if (okCheck2) { //오류가 안나야 true. try블록안에서 오류코드 다음의 코드는 진행이 안되는것을 활용.
                                        ...
                                        if (accountIdValueInteger > 0 && accountIdValueInteger <= userAccount.size()) { // 드디어 진입. /accounts/edit?accountId=1 같은 형식과 같음.
                                            ...
                                            if( session.loginingAccount.equals(userAccount.get(accountIdValueInteger-1)) ) { // 지금 로그인되어있는 계정명과 삭제하려는 계정명이 일치해야만 진입.

                                                String thisAccount = userAccount.get(accountIdValueInteger-1); // 해당 계정이름.

                                                session.userLogin = false; // 최상위 스코프에서 로그아웃처리.
                                                session.masterLogin = false; // 최상위 스코프에서 로그아웃처리.
                                                // 어차피 둘다 로그아웃일거라 둘다 false 해줘도 괜찮다.

                                                session.loginingAccount = null; // 로그아웃처리했으니 이 변수도 재할당.
                                                ...
                                                System.out.println("계정 삭제 권한이 없습니다.");
                                                elseCheck1 = true;
                                            }
