...
public class qwer2 {
    ...
    public static void main(String[] args) {
        ...
        userEditDay.add(null); // 계정 수정일 저장소에 null로 저장. 인덱스를 맞추기위해서.
        // --- 관리자 계정 초기에 추가 ---


        // 만약 user로그인에 성공했다면 session.userLogin == true인 상태로 프로그램(do문) 계속 실행됨.

        Session<String> session = new Session<>(null); // 유저로그인, 정보를 담는 session 객체를 최상위에서 생성
            ...
            if (!userInput.equals("종료")) {
                ...
                if (firstCheck && userInputPath.length == 3 && userInputPath[0].equals("") &&
                    ...
                    if (userInputPath[1].equals("boards") && userInputPath[2].equals("add") ) {
                        //  게시판 작성. 입력이 /boards/add   일경우에만 진입할것이다.
